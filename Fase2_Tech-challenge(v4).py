...
look_back = 60
X, Y = create_dataset(ibovespa_scaled, look_back)

# Dividir os dados em treino e teste de forma aleatória
X_train, X_test, Y_train, Y_test, dates_train, dates_test = train_test_split(X, Y, ibovespa.index[look_back:len(X) + look_back], test_size=0.2, random_state=42)
...
# Treinar o modelo
history = model.fit(X_train, Y_train, epochs=1, batch_size=1, validation_split=0.2, verbose=2, callbacks=[early_stopping]) #Aumentar epochs para aumentar acurácia (reduz desempenho)
...
test_predict_plot[len(train_predict) + (look_back * 2) + 1:len(ibovespa_scaled) - 1, :] = test_predict[:len(test_predict_plot[len(train_predict) + (look_back * 2) + 1:len(ibovespa_scaled) - 1])]

# Ajustar o índice para os arrays de previsão
train_dates = dates[look_back:len(train_predict) + look_back]
test_dates = dates[len(train_predict) + (look_back * 2) + 1:len(dates) - 1]

# Corrigir o índice de test_predict_plot e o tamanho de test_dates
test_predict_plot = np.empty_like(ibovespa_scaled)
test_predict_plot[:, :] = np.nan
test_predict_plot[len(train_predict) + (look_back * 2) + 1:len(dates) - 1, :] = test_predict[:len(dates) - 1 - (len(train_predict) + (look_back * 2) + 1)]
...
plt.plot(test_dates, test_predict[:len(test_dates)], label='Previsão de Teste')
